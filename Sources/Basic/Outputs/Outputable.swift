...
public protocol Outputable {
    ...
    /// - Returns: Outputable
    func filtering<T: Outputable>(_ filter: C7FilterProtocol?...) async -> T
    
    /// Filter processing
    /// - Parameters:
    ///   - filter: It must be an object implementing C7FilterProtocol
    /// - Returns: Outputable
    func make<T: Outputable>(filter: C7FilterProtocol) async throws -> T
    ...
    /// - Returns: Outputable
    func makeGroup<T: Outputable>(filters: [C7FilterProtocol]) async throws -> T
}

extension Outputable {
    
    public func filtering<T>(_ filter: C7FilterProtocol?...) async -> T where T : Outputable {
        do {
            let filters = filter.compactMap { $0 }
            let dest = HarbethIO.init(element: self, filters: filters)
            return try await dest.output() as! T
        } catch {
            return self as! T
        }
    }
    
    public func make<T>(filter: C7FilterProtocol) async throws -> T where T : Outputable {
        let dest = HarbethIO.init(element: self, filter: filter)
        return try await dest.output() as! T
    }
    
    public func makeGroup<T>(filters: [C7FilterProtocol]) async throws -> T where T : Outputable {
        let dest = HarbethIO.init(element: self, filters: filters)
        return try await dest.output() as! T
    }
}
